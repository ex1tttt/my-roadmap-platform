  ...
  return (
    <div ref={setNodeRef} style={style} className="box-border w-full rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900/50 p-4">
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        ...
        {/* đčĐÇđ░đ▓đ░ĐĆ đ║đżđ╗đżđŻđ║đ░: đ╝đÁđ┤đŞđ░ */}
...
export default function EditPage() {
  ...
  async function handleUpdate(e: React.FormEvent) {
    ...
    try {
      ...
      // 1) đ×đ▒đŻđżđ▓đ╗ĐĆđÁđ╝ đ║đ░ĐÇĐéđżĐçđ║Đâ
      // is_private đ╝đÁđŻĐĆđÁĐé Đéđżđ╗Đîđ║đż đ▓đ╗đ░đ┤đÁđ╗đÁĐć; đ║đżđ╗đ╗đ░đ▒đżĐÇđ░ĐéđżĐÇđ░đ╝ ĐŹĐéđ░ đ║đżđ╗đżđŻđ║đ░ đŻđÁđ┤đżĐüĐéĐâđ┐đŻđ░
      ...
      // đčĐÇđżđ▓đÁĐÇĐĆđÁđ╝ đ┤đżĐüĐéđŞđÂđÁđŻđŞĐĆ đĚđ░ đ║đżđ╗đŞĐçđÁĐüĐéđ▓đż đ║đ░ĐÇĐéđżĐçđÁđ║
      if (isOwner && user) {
        const { checkAndAwardBadges } = await import('@/lib/badges');
        await checkAndAwardBadges(user.id, 'first_card');
      }

      // 2) đíđŞđŻĐůĐÇđżđŻđŞđĚđŞĐÇĐâđÁđ╝ Đłđ░đ│đŞ: Đâđ┤đ░đ╗ĐĆđÁđ╝ ĐüĐéđ░ĐÇĐőđÁ Ôćĺ đ▓ĐüĐéđ░đ▓đ╗ĐĆđÁđ╝ đŻđżđ▓ĐőđÁ
      ...
      // 3) đíđŞđŻĐůĐÇđżđŻđŞđĚđŞĐÇĐâđÁđ╝ ĐÇđÁĐüĐâĐÇĐüĐő: Đâđ┤đ░đ╗ĐĆđÁđ╝ ĐüĐéđ░ĐÇĐőđÁ Ôćĺ đ▓ĐüĐéđ░đ▓đ╗ĐĆđÁđ╝ đŻđżđ▓ĐőđÁ
  ...
  async function handleDeleteImage(stepId: string, imageUrl: string) {
    try {
      ...
      alert("đ×ĐłđŞđ▒đ║đ░ Đâđ┤đ░đ╗đÁđŻđŞĐĆ đŞđĚđżđ▒ĐÇđ░đÂđÁđŻđŞĐĆ: " + (err as any)?.message);
    }
  }
    ...
    <div className="min-h-screen bg-white dark:bg-[#020617] py-12 px-6 text-slate-900 dark:text-slate-100">
      <main className="mx-auto max-w-4xl">
        ...
        <form onSubmit={handleUpdate} className="space-y-6">
          {/* đ×ĐüđŻđżđ▓đŻđ░ĐĆ đŞđŻĐäđżĐÇđ╝đ░ĐćđŞĐĆ */}
          <section className="rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900/50 p-6">
            ...
            {/* đčĐÇđŞđ▓đ░ĐéđŻđżĐüĐéĐî ÔÇö Đéđżđ╗Đîđ║đż đ┤đ╗ĐĆ đ▓đ╗đ░đ┤đÁđ╗ĐîĐćđ░ */}
            {isOwner && (
            ...
            {/* đúđ┐ĐÇđ░đ▓đ╗đÁđŻđŞđÁ đ┤đżĐüĐéĐâđ┐đżđ╝ ÔÇö đ▓đŻĐâĐéĐÇđŞ Đéđżđ│đż đÂđÁ đ▒đżđ║Đüđ░ */}
            {isOwner && (
