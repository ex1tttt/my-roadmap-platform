      ...
      const [{ data: { user } }, cardResponse] = await Promise.all([
        ...
        (async () => {
          ...
          if (isOldFormat) {
            ...
            // Это slug - ищем по slug, с fallback на id
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex gap-2 items-center">
            <ShareButton cardId={card.id} slug={card.slug} title={card.title} />
            {isOwner && <Link href={`/card/${card.id}/edit`} className="p-2 border border-white/10 rounded-lg"><Pencil size={16}/></Link>}
            {!isOwner && currentUser && <ReportCardButton cardId={card.id} />}
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-[1fr_300px] gap-12">
          <section>
            <StepsProgress cardId={card.id} userId={currentUser?.id} steps={steps} initialDone={initialDone} />
          </section>
