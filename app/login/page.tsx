...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      // router.refresh() заставляет Next.js перечитать куки сессии на сервере,
      // после чего middleware увидит актуальную сессию
      router.refresh();
      router.push("/");
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-6 dark:bg-black">
      <main className="mx-auto max-w-md">
        <h1 className="mb-4 text-2xl font-semibold text-gray-900 dark:text-gray-100">Войти</h1>
        <form onSubmit={handleLogin} className="space-y-4 rounded-lg bg-white p-6 shadow-sm dark:bg-gray-900">
